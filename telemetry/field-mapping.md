...
| `mcp.authz.grant_snapshot_hash`, `mcp.authz.grant_expiry` | Track 3 has no MCP-side representation of "what authorization justified this subscription" — this is entirely an audit-pipeline construct fed by the server's own token/grant state. **Correction (scope-aware pass):** `grant_snapshot_hash` is drift-hinting only and MUST NOT be used as an identity/join key — see `mcp.authz.binding_id` |
| `mcp.subscription.instance_id` (scope-aware pass addition) | MCP's `subscriptions/listen` pattern defines the wire subscription id as literally the JSON-RPC request `id` — connection-scoped only, explicitly not preserved across reconnects (`.../basic/patterns/subscriptions`: "the server holds no subscription state across reconnections"). This project needs a durable, cross-reconnect/cross-tenant/cross-server join key that the wire simply does not provide |
| `mcp.authz.binding_id` (scope-aware pass addition) | Neither MCP nor OAuth 2.0 (RFC 6749/7009) exposes a stable, wire-visible identifier for "this specific grant/session," independent of the token's own value — RFC 6749 §6 confirms a refresh issues a new token value for what is conventionally the same grant. This project needs that missing stable identifier to scope revocation correctly, instead of treating principal identity as a proxy for grant identity |
| `mcp.subscription.required_scope`, `mcp.authz.change.removed_scope` (scope-aware pass addition) | MCP's authorization model requires servers to reason about scope hierarchies per operation (`.../basic/authorization`) but defines no canonical scope-to-subscription-filter mapping; this project needs its own minimal scope-tag vocabulary to determine whether a scope downgrade is actually relevant to a given subscription instance, rather than assuming any downgrade invalidates every open stream for that principal |
| `mcp.authz.change.affected_scope`, `mcp.authz.change.affected_binding_ids` (scope-aware pass addition) | MCP defines no authorization-changed event at all (see the next row); this project further needs an explicit way for that invented event to state which binding(s) it affects, since OAuth token revocation (RFC 7009) treats cascading beyond a single token as a server-policy choice, never an automatic consequence of who the caller is |
...
| 3. Project-defined | ~35 (patched, scope-aware pass) | `mcp.authz.decision`, `mcp.task.authz_context_id_hash`, `mcp.subscription.authorization_change.*`, `mcp.validation.*`, `security.hash.key_id`/`algorithm`, `mcp.subscription.instance_id`, `mcp.authz.binding_id`, `mcp.authz.change.affected_scope`/`affected_binding_ids`/`removed_scope`, `mcp.subscription.required_scope` |
