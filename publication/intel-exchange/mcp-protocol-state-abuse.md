...
security-audit schema of its own), 98 deterministic test scenarios (519 events) across normal,
controlled-attack, and adversarial-stress corpora, 129 automated tests, and documented
false-positive/evasion analysis. All controlled-corpus metrics are explicitly not claimed as
real-world performance figures. (Three subsequent Track 3 remediation passes: (1) fixed two SPL
...
6 further fixtures. See `docs/validation-report.md` for the full account; re-run the test suite
...
- **Validation/stress** (`data/validation/`) — 67 scenarios, 319 events: adversarial-but-benign
...
regression-verified against the full suite (now 129 tests).
...
`tests/validation/language_equivalence.test.js`, zero disagreements across 98 scenarios). For
