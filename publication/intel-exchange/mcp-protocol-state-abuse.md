...
security-audit schema of its own), 92 deterministic test scenarios (485 events) across normal,
controlled-attack, and adversarial-stress corpora, 122 automated tests, and documented
false-positive/evasion analysis. All controlled-corpus metrics are explicitly not claimed as
real-world performance figures. (Two subsequent Track 3 remediation passes: (1) fixed two SPL
...
fixtures. See `docs/validation-report.md` for the full account; re-run the test suite rather
than assuming these exact counts stay fixed.)
...
- **Validation/stress** (`data/validation/`) — 61 scenarios, 285 events: adversarial-but-benign
...
regression-verified against the full suite (now 122 tests).
...
`tests/validation/language_equivalence.test.js`, zero disagreements across 92 scenarios). For
