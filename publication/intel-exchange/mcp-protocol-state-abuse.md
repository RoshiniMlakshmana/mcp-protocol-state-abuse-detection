...
project-defined telemetry fields, a three-outcome reporting model
(`confirmed_drift`/`evaluated_no_violation`/`insufficient_evidence`), and 13 further regression
fixtures; (3) an example-driven regression pass, built from ten independently-specified examples,
that removed a subtler unsound "sole-candidate" scope inference the second pass still carried,
replaced an "ever observed" approximation with a precise effective-time interval check, and added
...
in between, resolved against the SPECIFIC authorization binding backing that notification, not
merely "same principal" — a scope-aware correction verified against current MCP/OAuth
documentation (RFC 7009, RFC 6749, MCP's authorization/subscriptions pages). **Logic:** genuine
multi-event temporal correlation (KQL/SPL — see section 13 for why Sigma cannot fully express
this), reporting one of three outcomes per notification
(`ConfirmedDrift`/`EvaluatedNoViolation`/`InsufficientEvidence`) rather than a single boolean.
**Explicit deployment prerequisites:** the rule assumes no unmodeled permanent open-stream
exemption policy (confirmed, mechanically-verified operational false-positive source, fixture
V5-09) and depends on the new `mcp.authz.binding_id`/`mcp.subscription.required_scope`/
`mcp.authz.change.affected_scope` fields for fully resolved answers — a deployment that has not
instrumented them sees the safe `InsufficientEvidence` default on genuine ambiguity instead.
Full detail: `publication/detections-ai/detection-3-subscription-authorization-drift/metadata.md`.
...
- **Validation/stress** (`data/validation/`) — 75 scenarios, 351 events: adversarial-but-benign
...
could silently drop applicable matches), withdrew a vacuous "language equivalence" test that
compared one function's output to itself, and added 11 new regression fixtures (V11-01..V11-11)
— including one, V11-11, that empirically confirmed a previously reasoned-but-undemonstrated
cross-subscription scope boundary was real, at the time reported as unresolved.

**A second Track 3 remediation pass (scope-aware correction)** then fixed that exact boundary:
verified against current MCP/OAuth documentation, treating "same principal" as sufficient
revocation scope is a genuine category error (a principal can hold multiple independent,
independently-revocable authorization bindings). Fixed via six new project-defined telemetry
fields (`mcp.authz.binding_id`, `mcp.subscription.required_scope`,
`mcp.authz.change.affected_scope`/`affected_binding_ids`/`removed_scope`,
`mcp.subscription.instance_id`) and a three-outcome reporting model
(`confirmed_drift`/`evaluated_no_violation`/`insufficient_evidence`), with 13 further regression
fixtures (V12-01..V12-13). Fixture V11-11 is retained, unmodified, as a worked example of legacy
telemetry that still cannot be resolved without the new fields; fixture V12-13 is its corrected
counterpart. Full account: `docs/validation-report.md`, "Track 3 remediation pass" and "Track 3
remediation pass, part 2."

**A third Track 3 remediation pass (example-driven regression pass)** started from ten
independently-specified examples and found that the second pass's own fix still carried an
unsound "sole-candidate fallback" (an `affected_scope = unknown` change was resolved to a
confirmed finding whenever exactly one binding was observed — still an inference, not evidence)
and an "ever observed anywhere in the window" approximation for `affected_scope =
all_principal_bindings`. Both removed/replaced with a precise effective-time interval check;
a genuinely self-contradictory record shape (authoritative timing claimed with no `effective_at`)
is now explicitly detected rather than silently disappearing. Six further regression fixtures
were added (V13-01..V13-06), and fixture V6-02 is reclassified from a claimed "partially
detectable" finding to `insufficient_evidence`. Full account: `docs/validation-report.md`,
"Track 3 remediation pass, part 3."

**A fourth Track 3 remediation pass** targeted the one remaining documented Track 3
approximation: the SPL scope-downgrade relevance check compared only each side's FIRST scope tag
via an unanchored regex — a positional bug (a real overlap anywhere else went undetected) and a
substring-matching bug (an unanchored regex let `"files:read"` wrongly match inside
`"files:read_all"`) compounding each other. Inspecting the independently-coded SPL model in
`tests/validation/language_equivalence.test.js` found it had silently been implementing the
correct, exact intersection all along — the model was accidentally more correct than the query it
claimed to represent, so no prior "KQL and SPL agree" claim had ever actually exercised this bug.
Both are now fixed: the real SPL query computes an exact, order-independent,
anchored-literal-quoted multivalue intersection (`mvmap()`/`mvfind()`, syntax verified against
Splunk's documented Multivalue eval functions reference), and 8 further regression fixtures were
added (V14-01..V14-08). **One genuine, unresolved SPL/Splunk platform limitation was found and is
named, not approximated past**: classic Splunk field extraction cannot represent an
explicitly-empty scope list as distinct from an absent one, unlike KQL's `dynamic` type — fixture
V14-07 is asserted as one intentional, named KQL/SPL disagreement rather than folded into a
"fully equivalent" claim. Full account: `docs/validation-report.md`, "Track 3 remediation pass,
part 4."
...
- **V1-08** (Track 1): a collector that hashes a routing header without correctly decoding its
  ...
  underlying value.
- **V5-09** (Track 3): a permanent open-stream policy exemption is mechanically indistinguishable
  from a real violation, because the locked telemetry contract has no field for it. Tuning
  recommendation: a deployment-side allowlist, not a schema change. (V5-02, a grace-period
  scenario, is no longer in this bucket post-correction — see above; it now correctly reports
  `insufficient_evidence` rather than a mechanical, accepted false positive.)
