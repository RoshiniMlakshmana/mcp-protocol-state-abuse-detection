...
- **This detection requires custom MCP security audit instrumentation and field mapping** — see
  "Data source" and "Required fields" above, and `telemetry/field-mapping.md`. None of the
  required fields (including the new binding/scope fields) are emitted by Sentinel, Splunk, or
  any OpenTelemetry deployment by default.
- **An alert from this rule does not, by itself, prove compromise or successful unauthorized
  access** — even `ConfirmedDrift` reports continued *event visibility* past an authorization
  boundary, not confirmed content disclosure (see "Why it matters" above). See "False positives"
  and the "⚠️ Deployment prerequisite" note before treating a hit as confirmed malicious or
  enabling it as a paging alert.
- **KQL/SPL "equivalence," historically, means JS-model agreement, not native execution of
  either language.** Two independently-coded JS models of each language's own written semantics
  (`kqlModelResults`/`splModelResults` in `tests/validation/language_equivalence.test.js`) agree
  on outcome, row-for-row, across the full stress corpus, with exactly **one** intentional,
  named exception: fixture V14-07, an explicitly-empty (not absent) `required_scope`. KQL's
  `dynamic` type can represent "present but empty" distinctly from "absent"
  (`isempty(dynamic([]))` is documented `false`); classic Splunk field extraction cannot — so
  SPL reports `insufficient_evidence` there where the JS oracle and KQL correctly report
  `evaluated_no_violation`. **This divergence is real, documented, and preserved — it is not an
  artifact to be "fixed away."**
- **Native KQL execution, precisely:** 18 representative Track 3 test cases — `V11-01`, `V11-02`,
  `V11-03`, `V11-07`, `V12-01`, `V12-13`, `V13-01`, `V13-03`, `V13-05`, `V13-06`, and `V14-01`
  through `V14-08` — were separately, and later, executed against a real Kusto query engine:
...
- **Native SPL execution has not been performed for this detection. Zero SPL fixtures were
  ...
  unresponsive after a required configuration restart before any fixture could be tested; the
  root cause of the unresponsiveness is unconfirmed. See
  `evidence/native-execution/splunk-prep/STATUS.md` for the full account. The SPL query above
  remains verified only by hand-review and direct structural parallel to the natively-verified
  KQL query, per the "Query — SPL" section above.
- **Track 3 Sigma remains best-effort/hunting-only and is not semantically equivalent to
  KQL/SPL** — see "Query — Sigma" above for the full comparison matrix and limitation. This is
